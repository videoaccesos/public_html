// https://videoaccesos.net/reporte_personal_privadas.php?periodo=2023-09-07%20a%202024-09-01&privada=valencia&download=0
...
function procesar_registros($sql, $conexion, &$reportData, $renovacion, &$tarjetas_vistas) {
    ...
    if ($resultado->num_rows > 0) {
        while ($row = $resultado->fetch_assoc()) {
            ...
            $vencimiento_final = $fecha_vencimiento; // Valor por defecto

            // Buscar fecha en observaciones y ajustar vencimiento
            if (preg_match('/\b\d{2}\/\d{2}\/\d{4}\b/', $observaciones, $matches)) {
                $fecha_extraida = date('Y/m/d', strtotime(str_replace('/', '-', $matches[0])));
                
                // Si se encuentra "FOLIO", se agrega un año a la fecha extraída
                if (strpos($observaciones, 'FOLIO') !== false) {
                    $fecha_extraida_obj = new DateTime($fecha_extraida);
                    $fecha_extraida_obj->modify('+1 year');
                    $vencimiento_final = $fecha_extraida_obj->format('Y/m/d');
                }
                // Si se encuentra "VENCE" o "CAMBIO", se utiliza la fecha extraída sin modificación
                elseif (strpos($observaciones, 'VENCE') !== false || strpos($observaciones, 'CAMBIO') !== false) {
                    $vencimiento_final = $fecha_extraida;
                }
            }
