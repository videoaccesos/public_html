...
class Indicadores_Model extends CI_model
{
	...
	public function filtroTecnicos($strBusqueda, $intNumRows, $intPos){
		...
		$this->db->select("E.empleado_id, E.nombre, (SELECT OS.fecha_modificacion FROM ordenes_servicio as OS WHERE OS.tecnico_id = E.empleado_id AND OS.fecha_modificacion LIKE '%".$fechaActual."%' ORDER BY OS.fecha_modificacion DESC LIMIT 1) AS ultimo_reporte, (SELECT COUNT(*) FROM ordenes_servicio as OS WHERE OS.tecnico_id = E.empleado_id AND OS.fecha_modificacion LIKE '%".$fechaActual."%') AS total_reportes, (SELECT SUM(tiempo) FROM ordenes_servicio as OS WHERE OS.tecnico_id = E.empleado_id AND OS.fecha_modificacion LIKE '%".$fechaActual."%') AS tiempo_acumulado", FALSE);
		$this->db->from('empleados as E');
		$this->db->where('E.puesto_id', 6);
		$this->db->where("(SELECT COUNT(*) FROM ordenes_servicio as OS WHERE OS.tecnico_id = E.empleado_id AND OS.fecha_modificacion LIKE '%".$fechaActual."%') >", 0);
		$this->db->limit($intNumRows,$intPos);
		$res["indicadores"] =$this->db->get()->result();
		return $res;
	}
	...
	public function filtroReportes($strBusqueda, $intNumRows, $intPos){
		$fechaActual= date('Y-m-d');
		$this->db->like('usuario',$strBusqueda);
		$this->db->from('usuarios as u INNER JOIN empleados as e on u.empleado_id = e.empleado_id');
		$this->db->where('u.logueado', 1);
		...
		$this->db->select("(SELECT CONCAT(TIMESTAMPDIFF(DAY, OS.fecha_modificacion, NOW()), ' dia(s), ', MOD(TIMESTAMPDIFF(HOUR, OS.fecha_modificacion, NOW()), 24), ' hora(s) y ',MOD(TIMESTAMPDIFF(MINUTE, OS.fecha_modificacion, NOW()), 60), ' minuto(s) ')) AS tiempo_transcurrido, OS.folio, P.descripcion AS privada, OS.detalle_servicio AS detalle, OS.fecha_modificacion AS fecha, E.nombre as tecnico", FALSE);
		$this->db->from('ordenes_servicio AS OS INNER JOIN privadas as P on P.privada_id = OS.privada_id LEFT JOIN empleados as E on E.empleado_id = OS.tecnico_id');
		$this->db->where('OS.estatus_id', 1);
		$this->db->order_by('OS.fecha_modificacion','asc');
		$this->db->limit($intNumRows,$intPos);
		$res["indicadores"] =$this->db->get()->result();
		return $res;
	}
