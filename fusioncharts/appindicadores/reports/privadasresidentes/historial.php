<?php
		...
		$strSQL = " SELECT T.Tarjeta_id, RR.residente_id, P.descripcion AS privada,R.nro_casa, R.interfon,
		...
		while($row = mysql_fetch_object($result)) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue( "C$y", $row->nro_casa);
			$objPHPExcel->getActiveSheet()->setCellValue( "D$y", $row->interfon);
			$objPHPExcel->getActiveSheet()->setCellValue( "E$y", $row->email);
			$objPHPExcel->getActiveSheet()->setCellValue( "F$y", $row->telefono1);
			$objPHPExcel->getActiveSheet()->setCellValue( "G$y", $row->celular);
			$objPHPExcel->getActiveSheet()->setCellValue( "H$y", $row->no_tarjetas);
			$objPHPExcel->getActiveSheet()->setCellValue( "I$y", $row->estatus_id);
			$objPHPExcel->getActiveSheet()->getStyle('A'.$y.':I'.$y)->applyFromArray($est->RowText2);
		...
		$objPHPExcel->setActiveSheetIndex(0)
				    ->setCellValue('I'.$y,'Total: '. $intContador );
