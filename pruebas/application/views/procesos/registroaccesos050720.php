...
requestMethod.call(element);
 ...
 function fnAbrirVentanaRelayTarjeta2(intRenglon,intIndice){
    ...
    if(strTiempoActivacion=='')
    {
        ...
        // Aqui hacemos la peticion al servidor de Relays, utilizando la url mencionada
        // Mostramos mensaje al operador
        ...
        if ($resultado===FALSE){
            $("#divMensajes").html("Existe un problema de conexión con la tarjeta de relays!");
            //echo "Ha habido problemas de conexion con el servidor";
            ...
            $("#divMensajes").html("La instrucción se ejecutó exitosamente!");
            //echo "La instruccion se ejecuto exitosamente";
        }
    ...
    else
    {
        ...
        //abrimos la conexion
        $ch = curl_init();
        curl_setopt($ch,CURLOPT_URL, $strUrlPrueba);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        // //Si lo deseamos podemos recuperar la salida de la ejecución de la URL
        $resultado = curl_exec($ch);
        // //cerrar conexión
 ...
 var intContR=0;
  ...
  function fnVentana(strRelaysConcepto,intEstadoRelay,intCont,intNumeroRelays)
  {
    ...
    // hacemos un posteo mediante ajax para hacer el envio de la url
    setTimeout( function() {
        // Aqui hacemos la peticion al servidor de Relays, utilizando la url mencionada
        // Mostramos mensaje al operador
        $("#divMensajes").html("Procesando, espere por favor...");
        //Función que se utiliza para guardar la activación del relays
        $.ajax('procesos/posteo_relays.php',{
            "type" : "post",
            "data": {
                url:strUrl
            },
