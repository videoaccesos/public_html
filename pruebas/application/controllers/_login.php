...
class Login extends CI_Controller{
	...
	public function login_(){
        ...
        //Método para regresar la dirección IP anterior
        //del usuario que intenta logearse en el sistema
		public function regresarIPAnterior(){
		...
		//Método para regresar el estatus_verificacion  anterior del usuario 
		//que esta logeado en el sistema (para saber si se ha logeado en otro equipo y destruir la sesion actual)
		public function regresarCierreSesion(){
			$arrDatos= array('cierre_sesion'  =>  '',
				             'mensaje' => '');
			               		
			//Asignar valores de sesion
			$intUsuarioID=$this->session->userdata('usuario_id');
			$dteFecha=$this->session->userdata('fecha');

			//Seleccionar el estatus_verificacion anterior del usuario, para saber si inicio sesion desde otro equipo
			$strSQLSesion = "SELECT BI.cierre_sesion
						   	 FROM  bitacora_inicio  BI
						     WHERE BI.usuario_id=$intUsuarioID
						     AND   BI.inicio_sesion  = '$dteFecha'
						     AND   BI.cierre_sesion IS NOT NULL ";
						     
		    $resultSesion = $this->db->query($strSQLSesion);
		  	//Si existen registros
		   	if($row = $resultSesion->row()) {
		   		$arrDatos= array(
				                   'cierre_sesion'  =>  $row->cierre_sesion,
				                   'mensaje' => '' 
			               		);
